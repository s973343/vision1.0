...
def select_project_folder():
    ...
    print("Select a folder that contains MVLU/MLVU JSON files:")
...
def build_query_text(question, candidates):
    question = normalize_text(question)
    if isinstance(candidates, list) and candidates:
        opts = [stringify_candidate(x) for x in candidates]
        opts = [x for x in opts if x]
        if opts:
            if question:
                return f"{question} Options: {'; '.join(opts)}"
            return f"Options: {'; '.join(opts)}"
    return question


def extract_video_path(entry):
    return (
        entry.get("video")
...
def build_queries(folder):
    ...
    for json_path in json_files:
        ...
        for idx, entry in enumerate(entries):
            ...
            question = entry.get("question") or entry.get("query") or entry.get("prompt")
            candidates = entry.get("candidates") or entry.get("choices") or entry.get("options")
            answer_text = normalize_text(entry.get("answer"))
            query_text = build_query_text(question, candidates)

            if not query_text and not answer_text:
                continue

            video_value = normalize_text(extract_video_path(entry))
            row = {
                ...
                "query": query_text,
                "answer": answer_text,
                "question_type": normalize_text(entry.get("question_type")),
            }
            rows.append(row)
